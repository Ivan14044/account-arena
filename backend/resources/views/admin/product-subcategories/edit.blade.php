...
@section('title', 'Редактировать подкатегорию товаров')

@section('content_header')
    <div class="content-header-modern">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
            <div>
                <h1 class="m-0 font-weight-light">Редактировать подкатегорию товаров</h1>
                <p class="text-muted mb-0 mt-1 d-none d-md-block">Изменение данных и SEO-параметров подкатегории</p>
            </div>
            <div class="w-100 w-md-auto">
                <a href="{{ route('admin.product-subcategories.index') }}" class="btn btn-secondary btn-modern w-100 w-md-auto"><i class="fas fa-arrow-left mr-2"></i>Назад к списку</a>
            </div>
        </div>
    </div>
@stop
    ...
    <div class="row">
        ...
        <div class="col-lg-6">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.product-subcategories.update', $subcategory) }}">
                        @csrf
                        @method('PUT')
                                    ...
                                    @foreach (config('langs') as $code => $flag)
                                        <div class="tab-pane fade show {{ $code == 'ru' ? 'active' : null }}" id="content_{{ $code }}" role="tabpanel">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="name[{{ $code }}]" id="name_{{ $code }}" class="form-control @error('name.' . $code) is-invalid @enderror" value="{{ old('name.' . $code, $subcategoryData[$code]['name'] ?? '') }}">
                                                @error('name.' . $code)
                                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                                @enderror
                                            </div>
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="meta_title[{{ $code }}]" id="meta_title_{{ $code }}" class="form-control @error('meta_title.' . $code) is-invalid @enderror" value="{{ old('meta_title.' . $code, $subcategoryData[$code]['meta_title'] ?? '') }}">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="meta_description[{{ $code }}]" id="meta_description_{{ $code }}" class="form-control @error('meta_description.' . $code) is-invalid @enderror" value="{{ old('meta_description.' . $code, $subcategoryData[$code]['meta_description'] ?? '') }}">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <textarea style="height: 210px" name="text[{{ $code }}]" id="text_{{ $code }}" class="ckeditor form-control @error('text.' . $code) is-invalid @enderror">{!! old('text.' . $code, $subcategoryData[$code]['text'] ?? '') !!}</textarea>
                                                @error('text.' . $code)
                                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                                @enderror
                                            </div>
