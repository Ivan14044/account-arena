        ...
        foreach ($menu as $idx => $item) {
            if (is_array($item) && isset($item['id']) && $item['id'] === 'manual-delivery-count') {
                try {
                    ...
                    // ВАЖНО: Уменьшаем кеш до 1 секунды для максимальной актуальности при первой загрузке
                    $pendingCount = Cache::remember('manual_delivery_pending_count', 1, function () {
                        try {
