...
### Остаётся (опционально, требует согласования объёма)
- **Единый «почерк» хедеров:** формы create/edit используют дефолтный AdminLTE
  `content_header` (`<h1>`), тогда как страницы-списки — стилизованный
  `content-header-modern`. Унификация ~15 вью даст более цельный вид, но это
  поведение-нейтральный косметический рефактор большого объёма.
- **Stat-карточки:** на `users` мобильная раскладка 2×2 с иконкой сверху, на
  `service-accounts` — компактная горизонтальная. Обе самодостаточны; при желании
  можно привести к одному паттерну.
